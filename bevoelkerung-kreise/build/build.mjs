...
const TONSTUFEN = 8, WANDVON = 0.38, WANDBIS = 0.78;
const WANDFARBE = Array.from({ length: TONSTUFEN }, (unbenutzt, w) => {
  const f = WANDVON + (WANDBIS - WANDVON) * w / (TONSTUFEN - 1);
  return HYPSO.map((h, i) => 'rgb(' + Math.round(HYPSO_R[i] * f)
    + ',' + Math.round(HYPSO_G[i] * f) + ',' + Math.round(HYPSO_B[i] * f) + ')');
});
  ...
  for (let k = 0; k < N; k++) {
    ...
    if (k > 0) {
      ...
      for (let w = 0; w < schritte; w++) {
        const t = w / schritte;          // 0 ist der Fuss, 1 die Oberkante
        ctx.setTransform(a2, b2, c2, d2, eX,
          D * (ozY - (k - 1 + t) * dz) - b2 * cx - d2 * cy);
        ctx.fillStyle = WANDFARBE[Math.min(TONSTUFEN - 1, (t * TONSTUFEN) | 0)][kb];
