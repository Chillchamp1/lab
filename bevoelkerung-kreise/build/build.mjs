...
function reliefFeld() {
  ...
  if (w === rW && h === rH) return;
  rW = w; rH = h;
  for (const k of [hkA, hkB, hkC, hkF, hkP]) { k.width = w; k.height = h; }
  fBild = hcF.createImageData(w, h);
  pBild = hcP.createImageData(w, h);
...
function schraegMalen() {
  const W = rW, H = rH;
  const td = hcP.getImageData(0, 0, W, H).data;
  const po = pBild.data;
  po.fill(0);
  pQuelle.fill(-1);
  ...
  // Der Rahmen: die vier Ecken des Feldes, einmal flach und einmal in voller
  // Höhe. Daraus Massstab und Verschiebung, damit die Karte bei jedem Winkel
  // das Bild ausfüllt statt zu wandern.
  ...
  const z = Math.min((W - 2 * rand) / Math.max(1e-6, aMax - aMin),
                     (H - 2 * rand) / Math.max(1e-6, yMax - yMin));
  const oX = rand + (W - 2 * rand - (aMax - aMin) * z) / 2 - aMin * z;
  const oY = rand + (H - 2 * rand - (yMax - yMin) * z) / 2 - yMin * z;
  SICHT = { co, si, ct, st, cx, cy, hoch, z, oX, oY };

  // Ein Schritt in die Tiefe soll auf dem Bild rund einen Punkt weit sein.
  const db = Math.max(0.35, Math.min(2, 0.85 / Math.max(0.05, co * z)));
  const rock = Math.max(2, W / 130);     // dieselbe Kante wie flach, als Sockel
  ...
  const BLENDE = Math.max(3, W / 90);

  for (let x = 0; x < W; x++) { yPuffer[x] = H; pAngefangen[x] = 0; }
  ...
  for (let b = bMax; b >= bMin; b -= db) {
    ...
    for (let x = 0; x < W; x++) {
      ...
      const a = (x - oX) / z;
      const ix = (cx + a * ct + bx) | 0, iy = (cy + a * st + by) | 0;
      if (ix < 0 || iy < 0 || ix >= W || iy >= H) continue;
      const i = iy * W + ix, j = i << 2;
      if (td[j + 3] < 128) continue;               // draussen, kein Gelände
      const y = Math.round(yb - feldH[i] * hoch * si * z);
  ...
  hcP.putImageData(pBild, 0, 0);
  ctx.imageSmoothingEnabled = true;
  ctx.imageSmoothingQuality = 'high';
  ctx.drawImage(hkP, 0, 0, breite, hoehe);
}
...
function projPunkt(X, Y) {
  ...
  return [(a * SICHT.z + SICHT.oX) / s,
          ((b * SICHT.co - h * SICHT.hoch * SICHT.si) * SICHT.z + SICHT.oY) / s];
}
...
function reliefUeber(sil, deck) {
  ...
  const ziel = schraeg() ? hcP : hcF;
  ziel.setTransform(1, 0, 0, 1, 0, 0);
  ziel.globalCompositeOperation = 'source-over';
  ziel.globalAlpha = 1;
  ziel.putImageData(fBild, 0, 0);
  ziel.globalCompositeOperation = 'destination-in';   // nur, was auf der Karte liegt
  ziel.drawImage(hkA, 0, 0);
  ziel.globalCompositeOperation = 'source-over';
  if (schraeg()) {
    hoehenLinien(s, hcP, s);
    schraegMalen();
  } else {
    ctx.imageSmoothingEnabled = true;
    ctx.imageSmoothingQuality = 'high';
...
function linienMalen(strichBreite, zc, zm) {
  ...
  for (let e = 0; e < LINIENEIMER.length; e++) {
    ...
    zc.globalAlpha = Math.min(1, LINIE * st * (hell ? 1 : DUNKELLINIE));
    zc.stroke();
  }
  zc.globalAlpha = 1;
  zc.restore();
...
function hoehenLinien(s, zc, zm) {
  ...
  // Schräg wird die Linie in die Vorlage gemalt, also in ein gröberes Raster,
  // und die Schrägsicht staucht sie danach noch einmal. Ein Strich unter einem
  // Punkt Breite wird dabei zu Staub; er bekommt deshalb etwas mehr.
  const strichBreite = Math.max(0.7, breite / 760) * (zm !== 1 ? 1.25 : 1);
...
function zeigeTip(x, y) {
  ...
  if (schraeg() && pQuelle && rW > 0) {
    const s = rW / breite;
    const ix = Math.round(x * s), iy = Math.round(y * s);
    if (ix < 0 || iy < 0 || ix >= rW || iy >= rH) { versteckeTip(); stumm = -1; return; }
    const i = pQuelle[iy * rW + ix];
