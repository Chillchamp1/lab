...
/* Der Strich ist **grau, nicht schwarz** — und das ist der ganze Trick hinter
   den beleuchteten Kanten. Schwarz bliebe schwarz: soft-light hebt ein
   Schwarz kaum an. Ein mittleres Grau dagegen wird von der Lichtebene nach
   oben und nach unten gezogen, also weiss, wo die Sonne auf die Kante trifft,
   und dunkel, wo sie wegfällt. Das ist genau Tanakas beleuchtete Höhenlinie,
   die die flache Karte gezogen zeichnet — hier fällt sie als Nebenwirkung ab,
   ohne eine einzige zusätzliche Linie. */
let KANTENSTRICH = 0.9, KANTENDECK = 0.45, KANTENTON = '#777';
...
function scheibenMalen(s) {
  ...
  for (let k = 0; k < N; k++) {
    ...
    ctx.restore();
    /* Und die Kante als Linienzug darüber. Seit die Platten flach gefüllt
       werden, ist er die einzige Höhenlinie, die die Schrägsicht noch hat —
       Scheibenkante und Höhenlinie sind dasselbe Niveau. Er kostet nichts: es
       sind fünfundzwanzig Striche. */
    if (k > 0) {
      ctx.save();
      ctx.setTransform(a2, b2, c2, d2, eX, D * (ozY - k * dz) - b2 * cx - d2 * cy);
      ctx.lineJoin = 'round';
      ctx.lineWidth = KANTENSTRICH / zz;
      ctx.strokeStyle = KANTENTON;
