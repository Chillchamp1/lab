...
const WANDSTUFEN = [[0, 0.38], [0.34, 0.55], [0.67, 0.74]];
const WANDFARBE = WANDSTUFEN.map(([, f]) => HYPSO.map((h, i) =>
  'rgb(' + Math.round(HYPSO_R[i] * f) + ',' + Math.round(HYPSO_G[i] * f)
  + ',' + Math.round(HYPSO_B[i] * f) + ')'));
...
function scheibenMalen(s) {
  ...
  for (let k = 0; k < N; k++) {
    ...
    if (k > 0) {
      ...
      for (let w = 0; w < WANDSTUFEN.length; w++) {
        ctx.setTransform(a2, b2, c2, d2, eX,
          D * (ozY - (k - 1 + WANDSTUFEN[w][0]) * dz) - b2 * cx - d2 * cy);
        ctx.fillStyle = WANDFARBE[w][kb];
