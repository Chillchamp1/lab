...
<div class="modi" role="group" aria-label="What the colour shows">
  ...
  <button data-modus="dichte" aria-pressed="false">Density</button>
  <button data-modus="index" aria-pressed="false">Index</button>
</div>
...
<p>Area is always population: a county twice as populous is drawn twice as large.
Colour is what you switch. <b>People</b> repeats the population as a shade, so the
...
const FL = D.k.map(k => k[4]);
let basis = 0;   // Bild für den Index

function spanne(werte) {
  const s = werte.filter(v => v > 0).sort((a, b) => a - b);
  return [s[Math.floor(s.length * 0.02)], s[Math.floor(s.length * 0.98)]];
}
// Die Skalen gelten für beide Reihen gemeinsam, damit dieselbe Farbe beim
// Umschalten dasselbe bedeutet.
const alleWerte = REIHEN.flatMap(r => r.BEV.flatMap(b => [...b]));
const SP = {
  menschen: spanne(alleWerte),
  dichte: spanne(REIHEN.flatMap(r => r.BEV.flatMap(b => [...b].map((v, k) => FL[k] ? v / FL[k] : 0)))),
};

function farbe(modus, wert, k) {
  ...
  const u = (Math.log(wert) - Math.log(lo)) / (Math.log(hi) - Math.log(lo));
  return stufe(rampe(), Math.max(0, Math.min(1, u)));
}
...
function werteBei(a, b, u) {
  const w = new Float64Array(NK), deck = new Float64Array(NK);
  for (let k = 0; k < NK; k++) {
    const va = reihe.BEV[a][k], vb = reihe.BEV[b][k];
    if (va > 0 && vb > 0) { w[k] = va + (vb - va) * u; deck[k] = 1; }
...
function legende() {
  ...
  if (modus === 'index') {
    ...
    const [lo, hi] = SP[modus];
    li.textContent = nf.format(Math.round(lo)); re.textContent = nf.format(Math.round(hi));
    t.textContent = modus === 'menschen'
      ? 'People per county, logarithmic.'
      : 'People per square kilometre of actual land, logarithmic — the real area, not the stretched one.';
  }
}
...
function zeigeTip(x, y) {
  ...
  const bas = reihe.BEV[basis][treffer];
  ...
  tip.innerHTML = '<b>' + k[1] + '</b><dl>'
    ...
    + (bas > 0 ? '<dt>' + D.B[basis].jahr + ' = 100</dt><dd>' + Math.round(v / bas * 100) + '</dd>' : '')
    + '</dl>'
