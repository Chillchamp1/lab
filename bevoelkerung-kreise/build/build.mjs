...
const RAUF = 0.45;                // Auflösung des Höhenfelds, Anteil der Bildpunkte
...
function reliefFeld() {
  ...
  if (w === rW && h === rH) return;
  rW = w; rH = h;
  for (const k of [hkA, hkB, hkC, hkL]) { k.width = w; k.height = h; }
...
let LINIE = 0.9, NIVEAUS = 40, FLACHHANG = 0.006, DUNKELLINIE = 0.55;  // Höhenlinien: Stärke und Anzahl
...
function hoehenLinien(s) {
  ...
  const S = LINIENSCHRITT;
  const je = 1 / s;                       // Feldpunkt -> Bildpunkt
  const nx = Math.floor((rW - 1) / S), ny = Math.floor((rH - 1) / S);
  const F = grobH, M = maskeH;
  for (let cy = 0; cy < ny; cy++) {
    const y0 = cy * S, y1 = y0 + S;
    const r0 = y0 * rW, r1 = y1 * rW;
    for (let cx = 0; cx < nx; cx++) {
      const x0 = cx * S, x1 = x0 + S;
      // Nur dort, wo die Zelle ganz auf der Karte liegt. Das erspart das
      // Beschneiden an einem Pfad aus vierhundert Vielecken und lässt der
      // Küste einen schmalen, linienfreien Saum — der sieht ohnehin besser aus.
      ...
      const n0 = Math.ceil(lo * NIVEAUS), n1 = Math.floor(hi * NIVEAUS);
      if (n1 < n0) continue;

      // Gefälle der Zelle, je Feldpunkt. Daraus kommt alles Weitere.
      const gx = ((b + c) - (a + d)) / (2 * S), gy = ((d + c) - (a + b)) / (2 * S);
      const ql = Math.sqrt(gx * gx + gy * gy);
      if (!(ql > 1e-7)) continue;
      // Der Sonne zugewandt: +1, von ihr weg: −1.
      const f = (gx + gy) / (ql * Math.SQRT2);
      const hang = Math.min(1, ql / FLACHHANG);
      // Wie weit zwei Niveaus auf der Leinwand auseinanderliegen. Rücken sie
      // unter zwei Bildpunkte zusammen, verschmelzen die Linien zu einer
      // Fläche — dann blenden sie aus.
      const abstand = je / (ql * NIVEAUS);
      const eng = abstand > 4 ? 1 : Math.max(0, (abstand - 1.6) / 2.4);
      const st = Math.abs(f) * hang * eng;
      if (st < 0.025) continue;
      let k = (st * NSTUFE) | 0; if (k > NSTUFE - 1) k = NSTUFE - 1;
      const eimer = LINIENEIMER[f > 0 ? k : NSTUFE + k];

      const px0 = x0 * je, py0 = y0 * je, SS = S * je;
      for (let n = n0; n <= n1; n++) {
        const t = n / NIVEAUS;
        const ka = (a > t ? 1 : 0) | (b > t ? 2 : 0) | (c > t ? 4 : 0) | (d > t ? 8 : 0);
        if (ka === 0 || ka === 15) continue;
        // Schnittpunkte auf den vier Kanten, geradlinig eingerechnet.
        const e0x = px0 + SS * (t - a) / (b - a), e0y = py0;
        const e1x = px0 + SS,                     e1y = py0 + SS * (t - b) / (c - b);
        const e2x = px0 + SS * (t - d) / (c - d), e2y = py0 + SS;
        const e3x = px0,                          e3y = py0 + SS * (t - a) / (d - a);
        switch (ka) {
          case 1: case 14: eimer.push(e3x, e3y, e0x, e0y); break;
          case 2: case 13: eimer.push(e0x, e0y, e1x, e1y); break;
          case 3: case 12: eimer.push(e3x, e3y, e1x, e1y); break;
          case 4: case 11: eimer.push(e1x, e1y, e2x, e2y); break;
          case 6: case 9:  eimer.push(e0x, e0y, e2x, e2y); break;
          case 7: case 8:  eimer.push(e2x, e2y, e3x, e3y); break;
          default:         // 5 und 10: der Sattel, beide Strecken
            eimer.push(e3x, e3y, e0x, e0y, e1x, e1y, e2x, e2y);
        }
      }
    }
  }
  ...
  for (let e = 0; e < LINIENEIMER.length; e++) {
    ...
    ctx.lineWidth = (0.40 + 1.15 * st) * Math.max(0.7, breite / 760);
