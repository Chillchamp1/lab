...
let mBild = null, mW = 0, mH = 0, bandP = null, sortP = null;

function scheibenMalen() {
  const N = NBAND, W = rW, H = rH;
  if (mW !== W || mH !== H) {
    mW = W; mH = H;
    mkK.width = W; mkK.height = H;
    mBild = mkC.createImageData(W, H);
  ...
  const hd = hcF.getImageData(0, 0, W, H).data;
  const md = mBild.data;
  md.set(hd);
  const weich = k => {
    for (let q = zahl[k], e = zahl[k + 1]; q < e; q++) {
      const i = sortP[q], j = (i << 2) + 3;
      let v = farbF[i] * N - k;
  ...
  // Die Leinwand steht auf der Punktdichte des Geräts; setTransform ersetzt
  // sie, also muss sie in die Matrix hinein.
  ...
  const lagen = Math.max(1, Math.min(16, Math.ceil(dz)));
  ctx.imageSmoothingEnabled = true;
  ctx.imageSmoothingQuality = 'high';
  for (let k = 0; k < N; k++) {
    if (k > 0) { leer(k - 1); weich(k); }
    mkC.putImageData(mBild, 0, 0);
    for (let t = lagen - 1; t >= 0; t--) {
      ctx.setTransform(a2, b2, c2, d2,
        D * oX - a2 * cx - c2 * cy,
        D * (oY - (k - t / lagen) * dz) - b2 * cx - d2 * cy);
      ctx.drawImage(mkK, 0, 0);
    }
  }
...
function projPunkt(X, Y) {
  ...
  // Massstab und Versatz des Stapels stehen bereits in Punkten der Leinwand;
  // umzurechnen ist nur der Weg vom Feld in die Ansicht, nicht zurück.
...
/* Und zurück: welcher Punkt der flachen Karte liegt unter dem Zeiger?
   ...
   Beim Stapel ist es aber einfach: es sind nur fünfundzwanzig Scheiben, jede
   mit einer bekannten, umkehrbaren Abbildung. Also von oben nach unten
   durchgehen und die erste nehmen, deren Fläche den Punkt wirklich enthält —
   die oberste gewinnt, denn sie wurde zuletzt gemalt. */
function feldUnter(X, Y) {
  if (!SICHT || !(rW > 0)) return null;
  const S = SICHT, N = S.N;
  for (let k = N - 1; k >= 0; k--) {
    const u = (X - S.oX) / S.z, v = (Y - S.oY + k * S.dz) / (S.co * S.z);
    const fx = S.cx + S.ct * u - S.st * v, fy = S.cy + S.st * u + S.ct * v;
    const ix = fx | 0, iy = fy | 0;
    if (ix < 0 || iy < 0 || ix >= rW || iy >= rH) continue;
    const i = iy * rW + ix;
    if (!(maskeH[i] > 0.5)) continue;
    if (farbF[i] * N >= k) return [fx, fy];
  }
  return null;
}
