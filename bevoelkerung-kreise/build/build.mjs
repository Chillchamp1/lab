...
<div class="buehne" id="buehne">
  ...
  <div class="sicht">
    ...
    <label><span>Turn</span><input type="range" id="dreh" min="-180" max="180" value="0" step="1" aria-label="Turn the map"></label>
...
const hkP = document.createElement('canvas'), hcP = hkP.getContext('2d', { willReadFrequently: true });
const hkQ = document.createElement('canvas'), hcQ = hkQ.getContext('2d');
...
/* ---------- Der Blickwinkel ----------
   ...
   das ist kein neues Bild, sondern dasselbe: das Höhenfeld, aus dem Farbe,
   Schattierung und Höhenlinien schon kommen, wird nicht mehr platt
   hingelegt, sondern schräg angesehen.

   NEIGUNG 0 heisst senkrecht von oben — dann läuft alles wie bisher, Zeile
   für Zeile derselbe Weg, und der Film bleibt der alte. Erst ab der ersten
   Rastung gilt der andere Weg. */
...
/* ---------- Die Schrägsicht ----------
   Gerechnet wird wie in den alten Geländespielen: das Bild entsteht Spalte für
   Spalte, von vorn nach hinten, mit einem Horizont je Spalte. Wer hinten steht
   und nicht über diesen Horizont ragt, ist verdeckt und wird gar nicht erst
   gemalt. Jeder Bildpunkt wird genau einmal beschrieben, und die Verdeckung
   ist damit nicht geschätzt, sondern richtig.

   Die Vorlage ist die fertige Karte selbst — Farbe, Licht und Höhenlinien
   liegen schon darin. Nur die Höhe kommt getrennt dazu, aus feldH, demselben
   Feld, aus dem das Licht gerechnet wurde. Hochkant gestellt wird also nicht
   ein zweites Bild, sondern dieses.

   Gemalt wird in der Auflösung des Höhenfelds, nicht der Leinwand; das
   Hochrechnen am Ende macht dieselbe Glättung, die die flache Karte auch
   bekommt, und die Rechnung bleibt bei rund einer Viertelmillion Punkten. */
let SICHT = null;
let pW = 0, pH = 0;

/* ---------- Wie fein die Schrägsicht gemalt wird ----------
   Flach ist die Karte an zwei Stellen scharf, die das Höhenfeld nicht sind:
   die Höhenlinien und die Küste werden als Striche über das hochgerechnete
   Feld gezogen, in voller Auflösung der Leinwand. Die erste Fassung der
   Schrägsicht hat beides ins Feld gelegt und dann erst gekippt — damit war
   alles so grob wie das Feld, also 0,55 der Leinwand, und danach noch einmal
   vergrössert.

   Jetzt hat die Schrägsicht ihre **eigene** Auflösung, unabhängig vom Feld:
   die Farbe wird einmal auf diese Auflösung hochgerechnet, die Höhenlinien
   werden **darin** gezogen statt im Feld, und der Spaltenlauf malt Punkt für
   Punkt in dieser Auflösung. Nur die Höhe bleibt am Feld — die ist ohnehin
   weichgezeichnet und wird zwischen vier Nachbarn abgetastet, was die
   treppigen Flanken nimmt.

   Zwei Stufen, weil die feine kostet: **grob** (0,55 wie das Feld) beim
   Abspielen und solange jemand an einem Regler zieht, **fein** (bis zur
   Punktdichte des Geräts) sobald das Bild steht. Der Wechsel ist ein
   Nachzeichnen nach einer Fünftelsekunde. */
/* Zwei Bremsen, beide gemessen. Der grosse Sprung liegt zwischen 0,55 und 1,0
   der Leinwand — dort hört das Vergrössern auf. Darüber gewinnt nur noch ein
   Gerät mit dichten Bildpunkten etwas, und zwar wenig, während die Rechnung
   überproportional teurer wird: 0,60 Mio Punkte kosteten 130 ms, 0,75 Mio
   schon 215, 1,70 Mio dann 304.

   Der Deckel hängt deshalb an der **Punktzahl**, nicht am Gerät. Auf einem
   grossen Schirm mit dichten Punkten kostete das 1,4fache 85 ms mehr für
   zwölf Prozent mehr Auflösung; dort bleibt es bei der Leinwandgrösse. Auf
   einem Telefon, wo die Leinwand klein ist, passt das 1,4fache bequem unter
   den Deckel — und dort ist es auch zu sehen. */
const PFEIN = 1.4;
const PMAX = 0.62e6;
...
function sichtGrob() {
  ...
  clearTimeout(feinUhr);
  feinUhr = setTimeout(() => {
    if (!laeuft && schraeg()) { zeichne(); zeigeUmriss(letzterTip); }
  }, 230);
}

function projFeld(w, h) {
  if (w === pW && h === pH) return;
  pW = w; pH = h;
  hkP.width = w; hkP.height = h;
  hkQ.width = w; hkQ.height = h;
...
function schraegMalen(s) {
  /* Die Auflösung der Schrägsicht. Fein heisst: so fein wie die Leinwand
     wirklich ist, also samt Punktdichte des Geräts — dann trifft das Bild die
     Bildpunkte eins zu eins und wird beim Hinlegen nicht noch einmal
     verrechnet. */
  const dpr = Math.min(2.5, devicePixelRatio || 1);
  let faktor = RAUF;
  if (projFein()) {
    ...
    if (faktor < RAUF) faktor = RAUF;   // nie gröber als das Feld selbst
  }
  const w = Math.max(8, Math.round(breite * faktor)), h = Math.max(8, Math.round(hoehe * faktor));
  projFeld(w, h);
  const pS = pW / breite;            // Punkte der Schrägsicht je Punkt der Leinwand

  /* Die Vorlage, in **dieser** Auflösung. Die Farbe kommt hochgerechnet aus
     dem Feld — feiner ist sie nicht zu haben, sie ist ja weichgezeichnet —,
     die Höhenlinien dagegen werden hier frisch gezogen und sind damit so
     scharf wie auf der flachen Karte. Der Rand der Karte steckt schon im
     Alphakanal von hkF; die Schwelle weiter unten macht daraus wieder eine
     scharfe Küste, jetzt in der feinen Auflösung. */
  hcP.setTransform(1, 0, 0, 1, 0, 0);
  hcP.globalCompositeOperation = 'source-over';
  hcP.globalAlpha = 1;
  hcP.clearRect(0, 0, pW, pH);
  hcP.imageSmoothingEnabled = true;
  hcP.imageSmoothingQuality = 'high';
  hcP.drawImage(hkF, 0, 0, pW, pH);
  hoehenLinien(s, hcP, pS);
  const td = hcP.getImageData(0, 0, pW, pH).data;
  ...
  const cx = fW / 2, cy = fH / 2, hoch = fH * HOCH3D;
  const m = pS / s;                  // Punkte der Vorlage je Punkt des Feldes

  /* Der Kopfraum ist **fest**, nämlich das obere Ende der Farbleiter. Nach dem
     höchsten Berg zu rechnen, der gerade dasteht, wäre verlockend — 1871 gäbe
     es kaum einen, und die Karte stünde grösser da. Nur schrumpfte sie dann in
     dem Mass, in dem die Städte wachsen, und zwei Bilder wären nicht mehr
     zu vergleichen: genau das, wofür die ganze Karte gebaut ist. Also bleibt
     der Rahmen stehen und ein Teil davon 1871 eben leer. */
  const kopfraum = hoch;
  let aMin = 1e9, aMax = -1e9, bMin = 1e9, bMax = -1e9, yMin = 1e9, yMax = -1e9;
  for (const e of [[0, 0], [fW, 0], [0, fH], [fW, fH]]) {
    const dx = e[0] - cx, dy = e[1] - cy;
    const a = dx * ct + dy * st, b = -dx * st + dy * ct;
    if (a < aMin) aMin = a; if (a > aMax) aMax = a;
    if (b < bMin) bMin = b; if (b > bMax) bMax = b;
    const y0 = b * co, y1 = b * co - kopfraum * si;
    if (y1 < yMin) yMin = y1; if (y0 > yMax) yMax = y0;
  }
  const rand = 2 * pS;
  const z = Math.min((pW - 2 * rand) / Math.max(1e-6, aMax - aMin),
                     (pH - 2 * rand) / Math.max(1e-6, yMax - yMin));
  const oX = rand + (pW - 2 * rand - (aMax - aMin) * z) / 2 - aMin * z;
  ...
  /* Die Flanke wird nicht sofort dunkel, sondern über ein paar Punkte hinweg.
     Das ist kein Schönheitsgrund: ein Hang, der je Tiefenschritt einen Punkt
     fällt, bestünde sonst aus einer hellen und einer dunklen Zeile im Wechsel
     und sähe gesprenkelt aus. Erst eine wirkliche Wand — viele Punkte auf
     einen Schritt — soll als Wand erscheinen. */
  ...
  for (let b = bMax; b >= bMin; b -= db) {
    ...
    for (let x = 0; x < pW; x++) {
      ...
      /* Die Höhe zwischen vier Nachbarn. Rundete man sie auf den nächsten
         ...
         so gross wie ein Bildpunkt, also gut zu sehen. */
      const gx = fx > fWmax ? fWmax : fx, gy = fy > fHmax ? fHmax : fy;
      const x0 = gx | 0, y0 = gy | 0, ux = gx - x0, uy = gy - y0;
  ...
  hcQ.setTransform(1, 0, 0, 1, 0, 0);
  hcQ.globalCompositeOperation = 'source-over';
  hcQ.putImageData(pBild, 0, 0);
  ctx.imageSmoothingEnabled = true;
  ctx.imageSmoothingQuality = 'high';
  ctx.drawImage(hkQ, 0, 0, breite, hoehe);
}
...
/* Ein Punkt der flachen Karte, in der Schrägsicht. Gebraucht für die Namen und
   ...
   und müssen denselben Weg gehen wie das Gelände unter ihnen. */
function projPunkt(X, Y) {
  if (!SICHT || !(rW > 0)) return [X, Y];
  const s = rW / breite, pS = SICHT.pS;
  const fx = X * s, fy = Y * s;
  const ix = fx | 0, iy = fy | 0;
  const h = (ix >= 0 && iy >= 0 && ix < rW && iy < rH) ? feldH[iy * rW + ix] : 0;
  const dx = fx - SICHT.cx, dy = fy - SICHT.cy;
  const a = dx * SICHT.ct + dy * SICHT.st, b = -dx * SICHT.st + dy * SICHT.ct;
  return [(a * SICHT.z + SICHT.oX) / pS,
          ((b * SICHT.co - h * SICHT.hoch * SICHT.si) * SICHT.z + SICHT.oY) / pS];
}
...
function reliefUeber(sil, deck) {
  ...
  hcF.globalCompositeOperation = 'source-over';
  if (schraeg()) {
    // Schräg wird die Farbe erst in der Auflösung der Schrägsicht
    // zusammengesetzt; die Höhenlinien kommen dort hinein, nicht hierher.
    schraegMalen(s);
  } else {
    ctx.imageSmoothingEnabled = true;
    ctx.imageSmoothingQuality = 'high';
...
function zeigeTip(x, y) {
  ...
  if (schraeg() && pQuelle && SICHT && rW > 0) {
    const s = rW / breite, pS = SICHT.pS;
    const ix = Math.round(x * pS), iy = Math.round(y * pS);
    if (ix < 0 || iy < 0 || ix >= pW || iy >= pH) { versteckeTip(); stumm = -1; return; }
    const i = pQuelle[iy * pW + ix];
    if (i < 0) { versteckeTip(); stumm = -1; return; }
    x = (i % rW) / s; y = ((i / rW) | 0) / s;
  }
  let treffer = -1;
...
function halte() {
  ...
  // Steht das Bild, wird es scharf nachgezeichnet — beim Anhalten von Hand
  // ebenso wie am Ende des Durchlaufs.
  sichtGrob();
}
document.getElementById('spiel').onclick = () => laeuft ? halte() : starte();
document.getElementById('zeit').addEventListener('input', e => {
  halte(); sichtGrob(); setzeZeit(e.target.value / 1000); reliefFrisch(); zeichne();
});
...
document.getElementById('kipp').addEventListener('input', e => {
  NEIGUNG = e.target.value / 100; sichtGrob(); sichtNeu();
});
document.getElementById('dreh').addEventListener('input', e => {
  DREHUNG = e.target.value * Math.PI / 180; sichtGrob(); sichtNeu();
});
