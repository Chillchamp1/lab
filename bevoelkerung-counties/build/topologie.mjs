...
// Auf der Geometrieseite ist nichts zusammenzulegen: die County-Grenzen
// liegen bereits auf dem Stand, den die Karte zeigt (2020er Einteilung,
// Connecticut also noch mit acht Countys). Zusammengeführt wird nur auf der
// Datenseite, in `quellen.mjs` — dort steht auch, warum jede einzelne
// Zusammenführung exakt ist und nicht geschätzt.
export const GEBIETSSTAND = {};

// Mitte der Projektion: die Vereinigten Staaten ohne Alaska und Hawaii. Der
// Bezugspunkt 96 Grad West, 37,5 Grad Nord ist der des US National Atlas.
const LON0 = -96, LAT0 = 37.5;
...
export function baueKnotenmodell(kreise) {
  // Nach heutiger Kennziffer gruppieren
  ...
  for (let i = 0; i < lon.length; i++) { const [x, y] = laea(lon[i], lat[i], LON0, LAT0); X[i] = x; Y[i] = -y; }
