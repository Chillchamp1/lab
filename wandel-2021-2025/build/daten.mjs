...
export function leseErgebnisse() {
  ...
  for (const z of zeilen('kerg2.csv')) {
    ...
    const nr = Number(f[3]);
    if (!wk.has(nr)) wk.set(nr, { nr, name: f[4], landNr: f[6], stimmen: {}, vorp: {}, gueltig: 0, gueltigVorp: 0, wahlberechtigte: 0, waehlende: 0 });
    const e = wk.get(nr);
    const anzahl = zahl(f[11]);
    const vorher = zahl(f[13]);       // Vorperiode: Bundestagswahl 2021, amtlich
                                      // auf den Zuschnitt von 2025 umgerechnet
    if (f[7] === 'Partei') {
      let name = f[8];
      ...
      if (anzahl !== null) e.stimmen[name] = (e.stimmen[name] ?? 0) + anzahl;
      if (vorher !== null) e.vorp[name] = (e.vorp[name] ?? 0) + vorher;
    } else if (f[8] === 'Gültige') { e.gueltig = anzahl ?? 0; e.gueltigVorp = vorher ?? 0; }
    else if (f[8] === 'Wahlberechtigte') e.wahlberechtigte = anzahl ?? 0;
    else if (f[8] === 'Wählende') e.waehlende = anzahl ?? 0;
  }
  return wk;
}
